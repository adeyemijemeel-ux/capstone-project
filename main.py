...
# from hf import generate_response# main_part1 (Streamlit)
...
.card{border:1px solid #e6e6e6;background:#fff;border-radius: 10px; padding: 14px
16px;margin:10px 0;
box-shadow:0 1px 2px rgba(0,0,0,0.04);}
.q{font-weight:700; color:#0a6ebd;margin-bottom: 8px; }
.meta{display: inline-block;background: #FF9800; color:#fff;padding: 2px 8px;
...
def export_txt(history):
    ...
    enumerate(history, 1)])
    bio = io.BytesIO(txt.encode("utf-8")); bio.seek(e); return bio
...
def run_ai_teaching_assistant():
    ...
    if c1.button(" / Clear", key="c_ata"): st.session_state.history_ata = []; st.
    rerun()
    ...
    if st.button("Ask", key="a_ata"):
        if not q.strip(): st.warning(" A Enter a question.")
        else:
            with st.spinner("Thinking ... "):
                st.session_state.history_ata.append({"question": q.strip(),
...
def run_math_mastermind():
    ...
    c1, c2 = st.columns([1, 2])
    if c1.button(" Clear", key="c_mm"): st. session_state.history_mm = []; st.
    rerun()
    ...
    with st.form("mm_form", clear_on_submit=True):
        ...
        lv1 = b.selectbox("Level", ["Basic", "Intermediate", "Advanced"],index=1)
        if go:
            if not q.strip(): st.warning(" A Enter a problem.")
            else:
                with st.spinner("Solving ... "):
                    ans = math_answer(q.strip(), 1vl)
...
def main():
    ...
    opt = st. sidebar.selectbox("", ["AI Teaching Assistant", "Math Mastermind",
